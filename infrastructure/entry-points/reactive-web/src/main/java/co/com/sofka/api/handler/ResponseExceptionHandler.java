...
public class ResponseExceptionHandler {
	...
	public Mono<ResponseEntity<ProblemDetail>> manejarTodasExcepciones(Exception ex){
		registrarLog(ex);
	...
	public Mono<ResponseEntity<ProblemDetail>> manejarBaseException(BaseException ex) {
		registrarLog(ex);
	...
	public Mono<ResponseEntity<ProblemDetail>> manejarMethodNotAllowedException(MethodNotAllowedException ex){
		registrarLog(ex);
		final String FORMATO_MENSAJE = "El método HTTP %s no esta permitido para este endpoint";
		return Mono.just(ResponseEntity.status(METHOD_NOT_ALLOWED)
				.body(ProblemDetail.forStatusAndDetail(METHOD_NOT_ALLOWED,
						String.format(FORMATO_MENSAJE, ex.getHttpMethod()))));
	}

	@ExceptionHandler(ServerWebInputException.class)
	public Mono<ResponseEntity<ProblemDetail>> manejarServerWebInputException(ServerWebInputException ex){
		registrarLog(ex);
	...
	public Mono<ResponseEntity<ProblemDetail>> manejarUnsupportedMediaTypeStatusException(UnsupportedMediaTypeStatusException ex){
		registrarLog(ex);
		return Mono.just(ResponseEntity.status(UNSUPPORTED_MEDIA_TYPE)
